...
class imageSearch extends LitElement {
  ...
  // these are weird variable names with _ in them though they'll work and you used it consistently
  ...
  updated(changedProperties) {
    changedProperties.forEach((oldValue, propName) => {
      if (propName === 'term' && this[propName]) {
        ...
        (propName === 'year_start' || propName === 'year_end') &&
        this[propName]
      ) {
        this.getData();
      }
  ...
  async getData() {
    ...
    // There's... A LOT of complexity and duplication here.
    // no need for the New URL part here but you could just build an address

    /**
    let stuff = `https://images-api.nasa.gov/search?q=${this.term}&media_type=image&page=${this.page}`;
    if (this.year_start !== "") {
      stuff += `&year_start=${this.year_start}`;
    }
    
    if (this.year_end !== "")
    {
      stuff += `&year_end=${this.year_end}`;
    }
    */
    ...
    if (this.page !== '') {
      file = new URL(
        `https://images-api.nasa.gov/search?q=${this.term}&media_type=image&page=${this.page}`
      );
    }
    if (this.year_start !== '') {
      file = new URL(
        `https://images-api.nasa.gov/search?q=${this.term}&media_type=image&page=${this.page}&year_start=${this.year_start}`
      );
    }
    if (this.year_end !== '') {
      file = new URL(
        `https://images-api.nasa.gov/search?q=${this.term}&media_type=image&page=${this.page}&year_end=${this.year_end}`
      );
    }

    if (this.year_end !== '' && this.year_start !== '') {
      file = new URL(
        `https://images-api.nasa.gov/search?q=${this.term}&media_type=image&page=${this.page}&year_start=${this.year_start}&year_end=${this.year_end}`
      );
    }
      ...
      .then(data => {
        ...
        for (let i = 0; i < data.collection.items.length; i++) {
          ...
          const eventInfo = {
            ...
            secondary_creator:
              data.collection.items[i].data[0].secondary_creator,
          ...
          // debugging info should be removed from final solution
          console.log(file);
        }
  ...
  // view is not a variable in the properties block nor is it implemented in the demo
  // so there's effectively no way to run this with the display as a list of items only
  // the <br/> tag is unique and is "self closing" so there's no need to have this
  render() {
    return html`
